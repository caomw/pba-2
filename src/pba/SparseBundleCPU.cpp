...
namespace MYSSE
{
    ...
    /*inline float  sse_dot(__m128 s1, __m128 s2)
    {
        __m128 temp = _mm_dp_ps(s1, s2, 0xF1);
        float* f = (float*) (&temp); return f[0];
    }
    inline double  sse_dot(__m128d s1, __m128d s2)
    {
        __m128d temp = _mm_dp_pd(s1, s2, 0x31);
        double* f = (double*) (&temp); return f[0] ;
    }*/
...
namespace MYAVX
{
    template<class Float> class SSE{};
    template<>  class SSE<float>  {
        public: typedef  __m256 sse_type;
        //static size_t   step() {return 4;}
        static inline sse_type zero() {return _mm256_setzero_ps();}
    };

    template<>  class SSE<double> {
        public: typedef  __m256d sse_type;
        //static size_t   step() {return 2;}
...
namespace ProgramCPU
{
...
#if defined(CPUPBA_USE_SSE) || defined(CPUPBA_USE_AVX)
    ...
    void ComputeSQRT(avec<Float>& vec)
    {
        ...
        for(; p <= pex; p += step)
        {
            sse_store(p, sse_sqrt(sse_load(p)));
        }
#ifndef __GNUC__
        #pragma novector
#endif
    ...
    void ShuffleCameraJacobian(const avec<float>& jc, const vector<int>& map, avec<float>& result)
    {
        size_t num = jc.size() >> 4;
        for(size_t i = 0; i < num; ++i)
        {
            size_t ip = map[i];
            float* d = &result[i * 16];
            const float* s = &jc[ip * 16];
#ifdef CPUPBA_USE_AVX
            _mm256_store_ps(d, _mm256_load_ps(s));
            _mm256_store_ps(d + 8, _mm256_load_ps(s + 8));
#else
            _mm_store_ps(d, _mm_load_ps(s));
            _mm_store_ps(d + 4, _mm_load_ps(s + 4));
            _mm_store_ps(d + 8, _mm_load_ps(s + 8));
            _mm_store_ps(d + 12, _mm_load_ps(s + 12));
#endif
        }
    }
   void ShuffleCameraJacobian(const avec<double>& jc, const vector<int>& map, avec<double>& result)
    {
        size_t num = jc.size() >> 4;
        for(size_t i = 0; i < num; ++i)
        {
            size_t ip = map[i];
            double* d = &result[i * 16];
            const double* s = &jc[ip * 16];
#ifdef CPUPBA_USE_AVX
            for(int j = 0; j < 16; j += 4) _mm256_store_pd(d + j, _mm256_load_pd(s + j));
#else
            for(int j = 0; j< 16; j += 2) _mm_store_pd(d + j, _mm_load_pd(s + j));
#endif
        }
    }
    template <class Float>
    ...
    inline void   ScaleJ4(Float* jcx, Float* jcy, const Float* sj)
    {
        for(int i = 0; i < 4; ++i) {jcx[i] *= sj[i]; jcy[i] *= sj[i]; }
    }
    template <class Float>
    ...
    #define BEGIN_ADD_SCALED_VEC8(v)
    #define END_ADD_SCALED_VEC8(v)
        ...
        if(mt > 1 && (int)nproj >= mt)
        {
            ...
            for(int i = 0; i < (int)thread_num; ++i)
            {
                ...
                RUN_THREAD(ComputeJacobian, threads[i],
                    ...
                    intrinsic_fixed, radial_distortion, shuffle, jct, (int)first);
            }
            WAIT_THREAD(threads, thread_num);
        }else
        {
...
{
    if(dp)      return new SparseBundleCPU<double>;
    else        return new SparseBundleCPU<float>;
}
